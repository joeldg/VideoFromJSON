...
class APITestCase(unittest.TestCase):
    ...
    def test_create_video(self, mock_post):
        ...
        response = self.application.post(
            "/api/creation", json=data, headers={"X-API-Key": self.api_key}
        )
    ...
    def test_create_video_with_zoom_pan(self, mock_post):
        ...
        data = {"segments": segments, "zoom_pan": True}
        logger.debug(f"Sending request with data: {data}")
        response = self.application.post(
            "/api/creation", json=data, headers={"X-API-Key": self.api_key}
        )
    ...
    def test_create_video_with_fade_effect(self, mock_post):
        ...
        data = {"segments": segments, "fade_effect": "wipeleft"}
        logger.debug(f"Sending request with data: {data}")
        response = self.application.post(
            "/api/creation", json=data, headers={"X-API-Key": self.api_key}
        )
    ...
    def test_create_video_with_audiogram(self, mock_post):
        ...
        data = {"segments": segments, "audiogram": audiogram}
        logger.debug(f"Sending request with data: {data}")
        response = self.application.post(
            "/api/creation", json=data, headers={"X-API-Key": self.api_key}
        )
    ...
    def test_create_video_with_watermark(self, mock_post):
        ...
        watermark = {"text": "Sample Watermark", "position": "10:10", "opacity": 0.5}
        data = {"segments": segments, "watermark": watermark}
        logger.debug(f"Sending request with data: {data}")
        response = self.application.post(
            "/api/creation", json=data, headers={"X-API-Key": self.api_key}
        )
    ...
    def test_create_video_with_background_music(self, mock_post):
        ...
        data = {"segments": segments, "background_music": background_music}
        logger.debug(f"Sending request with data: {data}")
        response = self.application.post(
            "/api/creation", json=data, headers={"X-API-Key": self.api_key}
        )
    ...
    def test_invalid_api_key(self, mock_post):
        ...
        logger.debug(f"Sending request with data: {data}")
        response = self.application.post(
            "/api/creation", json=data, headers={"X-API-Key": "invalid_api_key"}
        )
        logger.debug(f"Received response: {response.data}")
    ...
    def test_create_video_with_custom_resolution(self, mock_post):
        ...
        data = {"segments": segments, "resolution": "1280x720"}
        logger.debug(f"Sending request with data: {data}")
        response = self.application.post(
            "/api/creation", json=data, headers={"X-API-Key": self.api_key}
        )
    ...
    def test_create_video_with_thumbnail(self, mock_post):
        ...
        data = {"segments": segments, "thumbnail": True}
        logger.debug(f"Sending request with data: {data}")
        response = self.application.post(
            "/api/creation", json=data, headers={"X-API-Key": self.api_key}
        )
    ...
    def test_create_video_with_audio_enhancement(self):
        ...
        data = {"segments": segments, "audio_enhancement": audio_enhancement}
        logger.debug(f"Sending request with data: {data}")
        response = self.application.post(
            "/api/creation", json=data, headers={"X-API-Key": self.api_key}
        )
    ...
    def test_create_video_with_dynamic_text(self, mock_post):
        ...
        data = {"segments": segments, "dynamic_text": dynamic_text}
        response = self.application.post(
            "/api/creation", json=data, headers={"X-API-Key": self.api_key}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_create_video_with_social_preset(self, mock_post):
        ...
        data = {"segments": segments, "social_preset": "tiktok"}
        response = self.application.post(
            "/api/creation", json=data, headers={"X-API-Key": self.api_key}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_create_video_with_template(self, mock_post):
        ...
        data = {"segments": segments, "template": "promo_template.mp4"}
        response = self.application.post(
            "/api/creation", json=data, headers={"X-API-Key": self.api_key}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_upload_pre_roll(self):
        ...
        data = {"file": (io.BytesIO(b"my file contents"), "pre_roll.mp4")}
        response = self.application.post(
    ...
    def test_upload_post_roll(self):
        ...
        data = {"file": (io.BytesIO(b"my file contents"), "post_roll.mp4")}
        response = self.application.post(
    ...
    def test_upload_image(self):
        ...
        data = {"file": (io.BytesIO(b"my image contents"), "test_image.png")}
        response = self.application.post(
            f"/api/upload_image/{directory}",
    ...
    def test_create_video_with_local_files(self, mock_post):
        ...
        data = {"segments": segments, "use_local_files": True}
        logger.debug(f"Sending request with data: {data}")
        response = self.application.post(
            "/api/creation", json=data, headers={"X-API-Key": self.api_key}
        )
